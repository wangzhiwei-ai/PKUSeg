...
namespace Program
{
    ...
    public class Matrix
    {
        ...
        public double[] saveMatrix()
        {
            double[] returnMatrix = new double[W.Length];
            for (int i = 0; i< W.Length; i++)
            {
                returnMatrix[i] = (double)W[i];
            }
           // returnMatrix = (double[])W.Clone();
            return returnMatrix;
        }
        ...
        public static Matrix newMatrix_random(int rows, int cols, double upbound)
        {
            ...
            {
                result.W[i] = (double)Global.randn.Sample() * upbound;
            }
            return result;
        }
        public static Matrix newMatrix_randomorthor(int rows, int cols, double upbound)
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    result.W[i * cols + j] = (double)xdata[i, j];
                }
            }
        ...
        {
            return newMatrix_val(rows, cols, (double)1.0);
        }
