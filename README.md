...
### 预训练模型
分词模式下，用户需要在seg/bin/x64/Release/model加载预训练好的模型，数据可以在https://drive.google.com/drive/folders/1X7Yfmfiakgt7DYE86IEFT1wZdrEhOasy 网址下载，根据具体需要，用户可以自定义的选择不同的预训练模型，以下是对不同模型的说明：
    msra_fast：fast模式下，在MSRA（新闻语料）上训练的模型
    msra_heavy：heavy模式下，在MSRA（新闻语料）上训练的模型
    ctb8 _fast： fast模式下，在CTB8（新闻文本及网络文本的混合型语料）上训练的模型
    ctb8_heavy：heavy模式下，在CTB8（新闻文本及网络文本的混合型语料）上训练的模型
    weibo_fast：fast模式下，在微博（网络文本语料）上训练的模型
    weibo_heavy：heavy模式下，在微博（网络文本语料）上训练的模型
    
若使用fast模式，请下载以fast为后缀结尾的模型并将文件夹下的内容放在seg/bin/x64/Release/model/fast文件夹下；若使用heavy模式，请下载以heavy为后缀的模型并将其内容放在seg/bin/x64/Release/model/heavy文件夹下。代码中的模型为不完整模型，仅供参考。其中MSRA数据由第二届国际汉语分词评测比赛提供（http://sighan.cs.uchicago.edu/bakeoff2005/ ），CTB8由LDC提供（https://catalog.ldc.upenn.edu/ldc2013t21   ），weibo数据由NLPCC分词比赛提供（http://tcci.ccf.org.cn/conference/2016/pages/page05_CFPTasks.html ）。
